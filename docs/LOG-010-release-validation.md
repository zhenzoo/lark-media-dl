...
## 2026-09-14 公开仓库与跨系统验收

- 已创建并推送公开仓库 [zhenzoo/lark-media-dl](https://github.com/zhenzoo/lark-media-dl)，main 本地与远端一致。公开文件检查未发现已知凭据、Cookie、个人应用标识或固定代理端口。
- 从 GitHub 重新克隆到独立目录，重新创建虚拟环境并安装。基础依赖不包含 oss2；默认 doctor 显示 TikHub Key 未配置、直连、无 Cookie，基础安装成功。
- 在该新副本的隔离 Agent home 注册 Skill，通过生成的包装入口实际下载 Threads 图文，得到 2 张 JPEG 和 1 个 TXT。该下载显式传入验收用配置文件，未依赖作者的个人 Skill 目录。
- 首轮 CI 中 Windows 短路径和 macOS 符号链接触发目录字符串比较误报，已改为 samefile 核验真实目录；其他下载和防覆盖断言保留。
- [GitHub Actions 验收](https://github.com/zhenzoo/lark-media-dl/actions/runs/34860616311)在提交 `899e441` 上全部成功：Windows、macOS、Linux × Python 3.10/3.13 的 6 组安装、19 项测试和 doctor，以及 1 组妙搭依赖安装、类型检查、合同测试和生产构建。
- 验收专用的隐藏 Worker 和隔离 Startup 项已停止、移除；已有生产 Worker 保持运行。没有重启整台电脑，也没有用云端 CI 代替真实登录自启动验收。

## 2026-09-14 单个有声视频与网页自动下载

- 复现旧下载 Skill 在 FFmpeg 不在 PATH 时，把独立视频流和音频流当作成功结果的问题；后续打包因此产生 ZIP。修复后先验证 FFmpeg 可执行，只接受 yt-dlp 后处理完成的单个 MP4，并检查应有的视频与音轨；提取音频模式单独输出 MP3。
- 同一条用户反馈的 X 视频，修复后实际输出一个 21,030,794 字节、约 76 秒的有声 MP4；音频模式实际输出单个 MP3。已有 Skill 修复同步至 claude-config 提交 `51f9228`，开源核心同步至 `24616b9`。
- 原工作台前端正式发布提交 `40c43ac`：保留“加入下载队列”，只跟踪本标签页新提交或重试的任务，完成后自动保存到当前设备。历史任务不自动下载；下载失败保留手动入口。未更改云端队列、生产 Worker 或 OSS 架构。
- 在已登录的桌面 Chrome 刷新正式网页，只提交一次原 X 链接；任务完成后浏览器自动下载 MP4，没有再次点击下载文件。浏览器落盘文件与 Worker 原文件 SHA-256 一致，FFmpeg 同时解码前 2 秒视频和音频成功；等待列表清空，未自动重下历史 ZIP。这是本机 Chrome 的验证，未代替其他电脑或手机浏览器验收。
- [修复后的 GitHub Actions](https://github.com/zhenzoo/lark-media-dl/actions/runs/34863312844)在 `907dd6f` 上七组全部成功：三系统六组安装、22 项 Python 测试与 doctor，以及网页 9 项测试、类型检查和生产构建。新增测试覆盖合并失败、缺少音轨、无效 FFmpeg、历史任务跳过、重复轮询、刷新恢复和下载失败降级。
